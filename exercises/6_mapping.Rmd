...
library(pals) # stevens.pinkgreen colors
...
# need a location column in the format "statename,countyname"
county_metadata <- county_metadata %>% 
  mutate(location = paste0(tolower(state.name),",",tolower(county.name)))

covid_county$location <- tolower(covid_county$location) %>% 
                                        stringr::str_replace("_",",") 

covid_county_wide <- covid_county %>% 
  filter(date == mapdate) %>% 
  pivot_wider(names_from = variable, values_from = value) %>% 
  select(location, Daily_Cases, Daily_Deaths) %>% 
  left_join(county_metadata, by="location") %>% 
  mutate(Daily_Cases_per100k = 100000 * Daily_Cases / population.2020) %>% 
  mutate(Daily_Deaths_per100k = 100000 * Daily_Deaths / population.2020) 

# Colors
colorfunction <- stevens.pinkgreen # bivariate color function
nbins <- 3

breaks.cases <- classIntervals(covid_county_wide$Daily_Cases_per100k, 
                               n=nbins,
                               style='equal') # equal breaks across range of data

breaks.deaths <- classIntervals(covid_county_wide$Daily_Deaths_per100k,
                               n=nbins,
                               style='equal') # equal breaks across range of data

covid_county_wide$class.cases <- findCols(breaks.cases)
covid_county_wide$class.deaths <- findCols(breaks.deaths)

covid_county_wide <- covid_county_wide %>% 
  mutate(class.joint = class.cases + nbins*(class.deaths-1))
